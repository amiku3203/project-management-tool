...
import { fetchTasks, setFilter, deleteTask, createTask, updateTask } from "../../redux/slices/taskSlice";
...
const ProjectDetails: React.FC = () => {
  ...
  const { tasks, loading: tasksLoading, filter } = useSelector((state: RootState) => state.tasks);
  ...
  useEffect(() => {
    if (id) {
      ...
      // Fetch all tasks for statistics (without filter)
      dispatch(fetchTasks({ projectId: id, status: undefined })).then((result: any) => {
        if (result.payload) {
  ...
  const handleFilterChange = (newFilter: 'all' | 'to-do' | 'in-progress' | 'done') => {
    dispatch(setFilter(newFilter));
  };

  const handleQuickStatusUpdate = async (taskId: string, newStatus: 'to-do' | 'in-progress' | 'done') => {
    if (id) {
      try {
        console.log('Updating task status:', { taskId, newStatus, projectId: id });
        ...
        // Refresh all tasks for statistics
        dispatch(fetchTasks({ projectId: id, status: undefined })).then((result: any) => {
          if (result.payload) {
            setAllTasks(result.payload);
          }
        ...
        // Refresh filtered tasks for display
        dispatch(fetchTasks({ projectId: id, status: filter === 'all' ? undefined : filter }));
      } catch (error) {
  ...
  const getTaskStats = () => {
    // Use allTasks for statistics to show total counts regardless of filter
    ...
    <div className="min-h-screen  bg-[url('/assets/bg3.png')] bg-cover bg-center">
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          ...
          {/* Filter and Create Task */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div className="flex space-x-2">
              {(['all', 'to-do', 'in-progress', 'done'] as const).map((filterOption) => (
                <button
                  key={filterOption}
          ...
          {tasksLoading ? (
            ...
            <div className="space-y-4">
              {tasks.map((task: Task) => (
                <div key={task._id} className="bg-gray-700 rounded-lg p-6 hover:bg-gray-600 transition duration-200  border-4 border-white">
                  <div className="flex justify-between items-start mb-4 ">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 text-sm">
                        <div className="flex items-center space-x-2">
                          ...
                          <select
                            value={task.status}
                            onChange={(e) => handleQuickStatusUpdate(task._id, e.target.value as 'to-do' | 'in-progress' | 'done')}
                    ...
                    <div className="flex space-x-2 ml-4">
                      <button
                        onClick={() => navigate(`/projects/${currentProject._id}/tasks/${task._id}/edit`)}
